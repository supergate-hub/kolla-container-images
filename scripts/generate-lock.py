...
def render_lock(
    ...
    lines = [
        ...
        "  schema_version: 2",
        f"  candidate_id: {yaml_string(summary['candidate_id'])}",
        f"  stream: {yaml_string(summary['stream'])}",
        f"  release: {yaml_string(summary['release'])}",
        f"  release_series: {yaml_string(summary['release_series'])}",
        f"  release_branch: {yaml_string(summary['release_branch'])}",
        "  release_metadata:",
        f"    repository: {yaml_string(summary['release_metadata']['repository'])}",
        f"    commit: {yaml_string(summary['release_metadata']['commit'])}",
        "  kolla:",
        f"    repository: {yaml_string(summary['kolla']['repository'])}",
        f"    version: {yaml_string(summary['kolla']['version'])}",
        f"    commit: {yaml_string(summary['kolla']['commit'])}",
        "  kolla_ansible:",
        f"    repository: {yaml_string(summary['kolla_ansible']['repository'])}",
        f"    version: {yaml_string(summary['kolla_ansible']['version'])}",
        f"    commit: {yaml_string(summary['kolla_ansible']['commit'])}",
        "  scope:",
        '    profile: "deployment"',
        '    image: "all"',
